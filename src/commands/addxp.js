...
module.exports = {
    data: new SlashCommandBuilder()
            .setName(`addxp`)
            .setDescription(`Permet d'ajouter des points d'expérience à un utilisateur`),
    ...
    async exec(message) {
        ...
        if (FunctionLibrary.hasAuthority(message)) {
            ...
            LevelModel.findOne({uid:userId}, (err, docs) => {
                ...
                user.xpGain(parseInt(xpToAdd))
                const newTotalXp = docs.totalXp + parseInt(xpToAdd)

                return message.reply(`**${userPing}** à gagner **${xpToAdd}** points d'xp. Iel est maintenant à **${newTotalXp}** points d'xp`)
            })
        } else {
